...
Браузерный генератор деталей передач для 3D-печати: зубчатого шкива GT2, гладкого шкива (ролика натяжителя или обводного ролика ремня), эвольвентной шестерни с прямыми, косыми или шевронными зубьями, прямозубой конической шестерни для валов под углом и зубчатой рейки с зубьями шестерни на параллельных валах. Параметры задаются по группам с поясняющими чертежами, деталь строится локально в фоновом потоке браузера. Результат виден в 3D и скачивается как STL. Основной интерфейс английский, русский — на странице `index_ru.html`.
...
| Коническая шестерня: модуль большого торца `m` / число зубьев `N` | 0,3…10 мм / 6…200 | 2 / 20 |
| Коническая шестерня: зубьев у ответной `N₂` / угол между валами `Σ` | 6…200 / 30…150° | 30 / 90 |
| Ширина зубчатой части, венца шестерни или обода `W` | 2…50 мм | 6 |
| Рейка: число зубьев `N` / высота до делительной прямой `H_p` | 2…200 / 1…100 мм | 20 / 8 |
...
80 тестов, около 15 секунд, браузер не нужен. Ядро проверяется на воспроизводимость, формулы профилей, связанные ограничения и инварианты сетки — ориентацию, замкнутость рёбер, одну связную оболочку, положительный объём; форма контуров и объём сверяются с аналитическим описанием, STL читается отдельным парсером. Тесты формы следят за тем, что у каждого видимого поля есть размер на чертеже его группы, а у каждой диагностики — текст, и что в английском и русском словарях одни и те же ключи. Тесты интеграции проверяют фоновый поток: совпадение с ядром, очередь запросов, отмену долгого построения и замену упавшего потока. Сквозная проверка в браузере выполняется вручную.
...
- `src/core/` — ядро без браузера и DOM: структура и диагностики (`parameters.js`), венцы и профили (`rims.js`, `involute.js`, `bevel.js`, `contours.js`), спицы, сборка и проверка сетки, геометрия чертежей, точка входа `generate.js`;
...
- Шестерня строится без скругления у основания зуба, настоящее подрезание ножки не моделируется: ниже основной окружности боковая сторона идёт по радиусу, при малом числе зубьев выдаётся предупреждение. Проверки пары — межосевого расстояния и зацепления — нет, одинаковые модуль, угол давления и величину наклона пары пользователь выдерживает сам. Рейка задаётся с теми же модулем, углом давления и наклоном, что её шестерня, наклон — с противоположным знаком; концы срезаны посередине впадины, так что рейки можно ставить встык. Крепёжных отверстий у рейки пока нет. Зацепление напечатанных шестерён и реек не проверялось.
- Коническая шестерня только прямозубая, по приближению Тредгольда, с высотой зуба, сходящейся к вершине конуса; спиральные зубья и постоянный радиальный зазор не поддерживаются. Большой торец — плоскость через окружность вершин, а не дополнительный конус. Ответная шестерня задаётся числом зубьев; одинаковую длину зуба вдоль конуса у пары пользователь выдерживает сам.
